...
export function buildReceiptHtml(res) {
  if (!res) return '';
  ...
  const menuBlock = res.menu ? `
    <div style="font-size: 12px; color: #444; display: grid; grid-template-columns: 1fr 1fr; gap: 4px; margin-top: 8px;">
      <div>• <strong>Chicken:</strong> ${esc(res.menu.chicken)}</div>
      <div>• <strong>Pork/Beef:</strong> ${esc(res.menu.beefPork)}</div>
      <div>• <strong>Fish/Seafood:</strong> ${esc(res.menu.fishSeafood)}</div>
      <div>• <strong>Vegetable:</strong> ${esc(res.menu.veg)}</div>
      <div>• <strong>Pasta:</strong> ${esc(res.menu.pasta)}</div>
      <div>• <strong>Staples:</strong> ${esc(res.menu.staples)}</div>
    </div>
  ...
  return `
    <div style="font-family: 'Outfit', sans-serif; color: #2D2825; max-width: 800px; margin: 0 auto; padding: 30px; border: 1px solid #E8E1D7; border-radius: 12px; background: #fff;">
      ...
      <div style="display: flex; justify-content: space-between; margin-bottom: 20px; font-size: 13px;">
        <div>
          <strong>Reference ID:</strong> #RES-${esc(res.id)}<br>
          <strong>Date Issued:</strong> ${esc(new Date().toLocaleDateString())}<br>
          <strong>Status:</strong> <span style="text-transform: uppercase; font-weight: bold;">${esc(res.status)}</span>
        </div>
        <div style="text-align: right;">
          <strong>Client Name:</strong> ${esc(res.name)}<br>
          <strong>Contact Number:</strong> ${esc(res.phone)}<br>
          <strong>Email Address:</strong> ${esc(res.email)}
        </div>
      </div>
      ...
      <div style="background: #FDF8F6; border: 1px solid #EED3CA; border-radius: 8px; padding: 15px; margin-bottom: 20px; font-size: 13px;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px;">
          <div><strong>Event Title:</strong> ${esc(res.eventTitle)}</div>
          <div><strong>Occasion:</strong> ${esc(res.eventType)}</div>
          <div><strong>Event Date & Time:</strong> ${esc(res.date)} @ ${esc(res.startTime)}</div>
          <div><strong>Venue:</strong> ${esc(res.venue)}</div>
          <div><strong>Theme / Motif:</strong> ${esc(res.theme || 'N/A')}</div>
          <div><strong>Centerpiece:</strong> ${esc(res.centerpiece || 'Artificial Flowers')}</div>
          <div style="grid-column: span 2;"><strong>Styro Name / Standee:</strong> ${esc(res.styroAvail ? res.styroName : 'Not Availed')}</div>
        </div>
      </div>
      ...
      <div style="margin-bottom: 20px; font-size: 13px;">
        ...
        <div style="border: 1px solid #E8E1D7; border-radius: 8px; padding: 12px; background: #fff;">
          <div style="display: flex; justify-content: space-between; font-weight: bold; margin-bottom: 8px; border-bottom: 1px solid #eee; padding-bottom: 6px;">
            <span>${esc(res.package.name)} (${esc(res.package.pax)} Pax)</span>
      ...
      <div style="display: flex; justify-content: space-between; margin-top: 40px; font-size: 12px; color: #666;">
        ...
        <div style="text-align: right;">
          <p style="margin: 0 0 30px 0;">Received by Client: <strong>${esc(res.name)}</strong></p>
...
export function printReceipt(res) {
  ...
  container.innerHTML = buildReceiptHtml(res);
  window.print();
  window.setTimeout(() => {
